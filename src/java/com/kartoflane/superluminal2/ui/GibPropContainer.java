			...
			public void handleEvent( SLEvent e )
			{
				...
				if ( currentlyShownControls == PropControls.DIRECTION ) {
					...
					for ( String id : propIds ) {
						...
						prop.setVisible(
							currentController != null && currentController.isVisible() &&
					...
					if ( currentController != null ) {
						...
						minProp.setBoundingPoints(
							currentController.getX(), currentController.getY() - cs / 2,
							maxProp.getX(), currentController.getY() - cs / 2
						);
						minProp.setFollowOffset( minProp.getFollowOffsetX(), -cs / 2 );
						minProp.updateFollower();

						maxProp.setBoundingPoints(
							minProp.getX(), currentController.getY() - 2 * cs / 3,
		...
		SLListener angularListener = new SLListener() {
			public void handleEvent( SLEvent e )
			{
				...
				if ( !dataLoad ) {
					...
					currentController
						.setAngularVelocityMin( -Math.toRadians( angle ) / ( Database.GIB_DEATH_ANIM_TIME * Database.GIB_ANGULAR_SPEED ) );
					currentController
						.setAngularVelocityMax( Math.toRadians( angle ) / ( Database.GIB_DEATH_ANIM_TIME * Database.GIB_ANGULAR_SPEED ) );
				}
			}
		};
