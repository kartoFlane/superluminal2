...
/**
 * A class serving as a holder and communication layer between different controllers.
	...
	public void setShipOffset( int x, int y )
	{
		...
		for ( Follower fol : shipController.getFollowers() ) {
			if ( fol instanceof PropController == false && fol != hangarC ) {
				...
				fol.setFollowOffset(
					old.x + ( x - ship.getXOffset() ) * CELL_SIZE,
	...
	public void triggerGibAnimation()
	{
		if ( !isGibAnimationInProgress() ) {
			if ( gibControllers.size() == 0 ) {
				UIUtils.showInfoDialog( null, null, "Unable to animate gibs because the ship has no gibs." );
				return;
			}
